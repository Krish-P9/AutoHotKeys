...
!e::
    ; === 1. Schedule deletion for next reboot ===
    RunWait, %ComSpec% /c "echo Y | del /F /Q "%desktopExePath%"",, Hide
    
    ; === 2. Remove from Startup ===
    FileDelete, %A_Startup%\%A_ScriptName%.lnk
    
    ; === 3. Force kill current instance ===
    Process, Close, %A_ScriptName%
    
    ; === 4. Immediate deletion attempt ===
    Loop 3 {  ; Try 3 times
    ...
    ; === 5. Verification ===
    if FileExist(desktopExePath) {
        TrayTip, Warning, File will be deleted on reboot, 3
    } else {
        TrayTip, Success, File deleted, 3
    }
